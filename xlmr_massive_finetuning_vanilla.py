...
training_order = args.task.split(",")
...
if len(args.previous_checkpoint) > 0:
     ...
     if args.backbone_model=='xlm-roberta-large':
         ...
         load_sharded_checkpoint(model, args.previous_checkpoint, strict=False)
# Finetuning pipeline
model.train()
    ...
    for task in training_order:
        ...
        if task == 'intent':
            ...
            for step in range(num_train_optimization_steps_per_epoch):
                examples, labels = random_sample_examples_for_intent(data_hf = train, data_idxes = train_idxes, batch_size = args.train_batch_size//2)
            ...
            for step in tqdm(range(num_train_optimization_steps_per_epoch)):
                examples = random_sample_exampels_for_slot(data_hf = train, data_idxes = train_idxes, batch_size = args.train_batch_size)
                features = tokenize_and_align_labels(examples = examples, tokenizer = tokenizer, max_seq_length = args.max_seq_length)
                ...
                optimizer.zero_grad()
                model.train()
                loss = model(features=features, task='slot', labels=features['labels'])[1]
                # backward pass
                loss.backward()
                ...
                if actual_steps > 0 and actual_steps % args.save_every_n_steps == 0:
    ...
    if (epoch+1) % 5 == 0:
      for task in training_order:
        if task=='intent':
            ...
            if best_result_intent < res_intent['acc']:
                best_result_intent = res_intent['acc']
                print("task intent result:", best_result_intent)
                if args.prefix:
                    if len(args.previous_checkpoint) > 0:
                        tmpf = args.previous_checkpoint.split('/')[0]
                        path = os.path.join(args.save_checkpoints_folder, f"Joint_Prefix_seed{args.seed}_headPred_MASSIVE_task_{task}_contine_{tmpf}_TrainLanguage_{args.language}_Intent_{args.training_strategy}_{args.pre_seq_len}_{args.epochs}_{args.learning_rate}")
                    else:
                        path = os.path.join(args.save_checkpoints_folder, f"Joint_Prefix_seed{args.seed}_headPred_MASSIVE_task_{task}_TrainLanguage_{args.language}_intent_{args.training_strategy}_{args.pre_seq_len}_{args.epochs}_{args.learning_rate}")

                else:
                    path = os.path.join(args.save_checkpoints_folder, f"Joint_seed{args.seed}_headPred_MASSIVE_task_{task}_TrainLanguage_{args.language}_Intent_{args.training_strategy}_{args.learning_rate}")
                if not os.path.isdir(path):
                        os.mkdir(path)
                model.save_pretrained(path)
            ...
            tmp_res = tmp_res/len(language_set)
            if best_result_slot < tmp_res:
                best_result_slot = tmp_res
                print("task slot result:", best_result_slot)
                if args.prefix:
                    if len(args.previous_checkpoint) > 0:
                        tmpf = args.previous_checkpoint.split('/')[0]
                        path = os.path.join(args.save_checkpoints_folder, f"new_seed{args.seed}_Prefix_MASSIVE_task_{task}_contine_{tmpf}_TrainLanguage_{args.language}_{args.training_strategy}_{args.hidden_dropout_prob}_{args.pre_seq_len}_{args.epochs}_{args.learning_rate}")
                    else:
                        path = os.path.join(args.save_checkpoints_folder, f"new_seed{args.seed}_Prefix_MASSIVE_task_{task}_TrainLanguage_{args.language}_Intent_{args.training_strategy}_{args.hidden_dropout_prob}_{args.backbone_model}_{args.pre_seq_len}_{args.epochs}_{args.learning_rate}")

                else:
                    path = os.path.join(args.save_checkpoints_folder, f"new_seed{args.seed}_MASSIVE_task_{task}_TrainLanguage_{args.language}_intent_slot_{args.backbone_model}_{args.training_strategy}_{args.learning_rate}")
                if not os.path.isdir(path):
                        os.mkdir(path)
                model.save_pretrained(path)
